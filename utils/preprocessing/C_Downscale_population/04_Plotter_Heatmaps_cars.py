...
# Load car flows
flow_file = Path("/soge-home/projects/mistral/miraca/processed_data/lifelines_OD/OD_vehicles/cars_OD.parquet")
flows_df = pd.read_parquet(flow_file)

# Load population nodes
pop_nodes_file = Path("/soge-home/projects/mistral/miraca/incoming_data/spatial_data/population_data/nodes_with_population_merged_consolidated_reduced.parquet")
...
print(flows_df.columns, flush=True)

# Ensure cars column exists
car_col = 'cars_vehicles' if 'cars_vehicles' in flows_df.columns else 'value'
if car_col not in flows_df.columns:
    raise ValueError(f"flows_df must contain '{car_col}' column.")
...
outgoing_flows = flows_df.groupby('origin_node', as_index=False)[car_col].sum()
outgoing_flows = outgoing_flows.rename(columns={car_col: 'outgoing_cars'})

# Aggregate incoming flows by dest_node
incoming_flows = flows_df.groupby('dest_node', as_index=False)[car_col].sum()
incoming_flows = incoming_flows.rename(columns={car_col: 'incoming_cars'})
...
pop_nodes_gdf['outgoing_cars'] = pop_nodes_gdf['outgoing_cars'].fillna(0.0)/365
pop_nodes_gdf['incoming_cars'] = pop_nodes_gdf['incoming_cars'].fillna(0.0)/365

print(f"Total outgoing cars: {pop_nodes_gdf['outgoing_cars'].sum():.0f}", flush=True)
print(f"Total incoming cars: {pop_nodes_gdf['incoming_cars'].sum():.0f}", flush=True)
...
# Output directory
out_dir = Path("/soge-home/projects/mistral/miraca/incoming_data/plots/heatmaps/cars")
out_dir.mkdir(parents=True, exist_ok=True)

# Generate heatmaps for outgoing and incoming cars
for flow_type in ['outgoing_cars', 'incoming_cars']:
    ...
    vmax_val = np.percentile(heatmap[heatmap > 0], 99) if (heatmap > 0).any() else heatmap.max()
    ...
    c = ax.imshow(
        ...
        origin="lower",
        cmap="ocean_r",  # Colormap for car flows
    ...
    cbar.set_label(f"{flow_type.replace('_', ' ').title()} (cars/day)", fontsize=12)
    
    # Add title and labels
    ax.set_title(f"Heatmap of {flow_type.replace('_', ' ').title()}", fontsize=16)
...
print("Car heatmap generation complete!", flush=True)
